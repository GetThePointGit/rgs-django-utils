...
class UserView(HasuraTrackedView):
    ...
    def get_permissions(cls):
        return {
            "config": TPerm(org_mem={
                "select": {"project": {"organization_id": {"_eq": "x-hasura-org-id"}} },
                "proj_read": {
                    "select": {"project": {"upm": {"_and": [
                        {"user_id": {"_eq": "x-hasura-org-id"}}, 
                        {"role": {"_lte": 140}} 
                    ]}}},
                }
            }),
    ...
    def get_sql(self):
        ...
        fields_refencing_user = [field for field in fields if field.is_relation and field.related_model == apps.get_model("core", "User") and isinstance(field, dj_models.ForeignKey)]
        if self.through_model != self.model:
            field_that_references_through_model = next((field for field in fields if field.is_relation and field.related_model == self.through_model and isinstance(field, dj_models.ForeignKey)), None)
            ...
            join_clause = ""
        on_clause = " OR ".join([f"au.id = {'tbl' if self.through_model == self.model else self.through_model._meta.db_table}.\"{field.name}_id\"" for field in fields_refencing_user])
    ...
    def get_all_views(cls) -> Generator[HasuraTrackedView, None, None]:
        ...
        for model in app_models:
            ...
            if not any(field.is_relation and field.related_model == apps.get_model("core", "User") and isinstance(field, dj_models.ForeignKey) for field in fields):
                continue
            ...
            if not any(field.is_relation and field.related_model == apps.get_model("core", "Project") and isinstance(field, dj_models.ForeignKey) and field.column == "project_id" for field in through_model_fields):
                continue
